...
describe("parseFEN", () => {
  ...
  it("should create the game board correctly", () => {
    ...
    for (let i = 0; i < 8; i++) {
      for (let j = 0; j < 8; j++) {
        ...
        expect(gameBoard.matrix[i][j]?.type).toEqual(expectedType);
        if (i < 2) {
          expect(gameBoard.matrix[i][j]?.color).toEqual("black");
        }
        if (i > 6) {
          expect(gameBoard.matrix[i][j]?.color).toEqual("white");
        }
      }
    }
  ...
  it("should parse castling correctly", () => {
    ...
    const threeCastle = "rnbqkbnr/pppppppp/8/8/8/8/PPPPPPPP/RNBQKBNR w Qkq - 0 1"
