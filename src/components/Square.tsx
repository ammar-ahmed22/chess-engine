...
const Square: React.FC<SquareProps> = ({
  ...
  const { whiteSquare, blackSquare, pieceSet, reversedBoard } = ctx.settings;

  const row = Math.floor(idx / 8);
  const col = idx % 8;
  const isWhite = (row + col) % 2 === 0;
  const image = piece ? pieceSets[pieceSet][piece.color][piece.type] : undefined;
  
  const id = SquareID.fromFlatIdx(idx, reversedBoard).toString();
